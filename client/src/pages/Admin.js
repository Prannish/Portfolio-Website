...
const Admin = () => {
  ...
  const handleResumeUpload = async e => {
    ...
    try {
      ...
      setResumeMessage('Upload failed');
    }
  };
  ...
  const handleResumeDelete = async () => {
    try {
      ...
      setResumeMessage('Deleted successfully!');
      fetchAll();
    } catch {
      setResumeMessage('Delete failed');
    }
  };
  ...
  const handleAddSkill = async e => {
    ...
    try {
      ...
      setSkillMessage('Add failed');
    }
  };

  const handleRemoveSkill = async skill => {
    try {
      await api.delete(`/skills/${skill}`);
      setSkillMessage('Skill removed!');
      fetchAll();
    } catch {
      setSkillMessage('Remove failed');
    }
  };
  ...
  const handleExperienceSubmit = async e => {
    e.preventDefault();
    try {
      if(editingExperience) await api.put(`/experiences/${editingExperience._id}`, experienceData);
      else await api.post('/experiences', experienceData);
      setExperienceData({ position:'', company:'', startDate:'', endDate:'', skills:'', description:'' });
      setEditingExperience(null);
      setExperienceMessage('Saved!');
      fetchAll();
    } catch {
      setExperienceMessage('Save failed');
    }
  };
  ...
  const handleCertSubmit = async e => {
    ...
    try {
      ...
      setCertData({ title:'', issuer:'', issueDate:'', url:'', image:null });
      fetchAll();
      setCertMessage('Saved!');
    } catch {
        ...
        {activeTab==='projects' && (
          <div className="section">
            ...
            <form onSubmit={handleProjectSubmit}>
              <input placeholder="Title" value={projectData.title} onChange={e=>setProjectData({...projectData,title:e.target.value})} required />
              ...
              <input placeholder="Technologies (comma)" value={projectData.technologies} onChange={e=>setProjectData({...projectData,technologies:e.target.value})} />
              <input placeholder="Github URL" value={projectData.githubUrl} onChange={e=>setProjectData({...projectData,githubUrl:e.target.value})} />
              <input placeholder="Live URL" value={projectData.liveUrl} onChange={e=>setProjectData({...projectData,liveUrl:e.target.value})} />
              <label>
                <input type="checkbox" checked={projectData.featured} onChange={e=>setProjectData({...projectData,featured:e.target.checked})} /> Featured
              </label>
              <input type="file" onChange={e=>setProjectData({...projectData,image:e.target.files[0]})} accept="image/*" />
